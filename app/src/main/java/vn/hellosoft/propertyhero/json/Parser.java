    ...
    public static List<Account> accountList(JSONObject response) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                accounts.add(new Account(id, userName, fullName, gender, vn.hellosoft.helper.Utils.strToDate(birthDate), phoneNumber, email, address, countryID, provinceID, districtID,
                        avatar, idCode, vn.hellosoft.helper.Utils.strToDate(issuedDate), issuedPlace, accRole, accType, status));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public static List<Notify> notifyList(JSONObject response) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                notifies.add(new Notify(id, name, content, thumb, url, vn.hellosoft.helper.Utils.strToDate(date)));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
